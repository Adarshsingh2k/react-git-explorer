    ...
    this.setState({loading: true}, async ()=>{
      ...
      this.fetchRepos(username)]
        ...
        if(user.data !== undefined && repos.data!==undefined){
         return this.setState({user: user.data,
          repos:repos.data,
          page: repos.page,
          loading: false})
        }
...
loadMore= async ()=>{
  ...
  render(){
    ...
    {(loading && <p><img src="https://img.icons8.com/officel/2x/spinner-frame-5.png"></img> </p>)}
   ...
   {!loading && !reposError && repos.map(repo=> <RepoCard key={repo.id} repo={repo}/>)}
    ...
    (<button className="btn btn-success" onClick={this.loadMore}>More</button>)
    }
